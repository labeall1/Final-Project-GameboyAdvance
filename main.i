...
OBSTACLE jellyfish[10];
OBSTACLE obstacles2[10];
 ...
 *(unsigned short *)0x4000000 = 0 | (1<<8) | (1<<9) | (1 << 12);
 ...
 playSoundA(backgroundSong, 213249, 11025, 1);
 ...
 for(int i = 0; i < 10; i++)
 {
  ...
  if(jellyfish[i].active)
  {
   ...
   if(catchJellyfish(&spongebob, &jellyfish[i]) && spongebob.aniState == 8)
   {
    ...
    plusFive.active = 1;
    plusFive.row = ((spongebob.row) >> 8) - 12;
    ...
    if(caughtFish == 5)
    {
     playSoundB(laugh, 19285, 11025, 0);
    }
...
void clearObstacles()
{
 for(int i = 0; i < 10; i++)
 {
  ...
  if(!(obs->active))
  {
   shadowOAM[ENEMY + i].attr0 = (2 << 8);
  }

  if(!(jelly->active))
  {
   shadowOAM[JELLYFISH + i].attr0 = (2 << 8);
  }

  if(!(obs2->active))
  {
   shadowOAM[JELLYFISH2 + i].attr0 = (2 << 8);
  }
 }
}
...
void game()
{
 ...
 if((!(~(oldButtons)&((1<<1))) && (~buttons & ((1<<1)))) && !cheat)
 {
  health = 5;
  cheat = 1;
 }
 ...
 *(volatile unsigned short*)0x4000008 = 0 << 2 | 31 << 8 | 0<<14 | 1 << 7;
 loadPalette(splashPal);
 DMANow(3, splashTiles, &((charblock *)0x6000000)[0], 12288/2);
 ...
 *(unsigned short*)0x4000050 = (1 << 9) | (1 << 8) | (1 << 12) | (1 << 13) | (1 << 6);
 ...
 *(volatile unsigned short*)0x400000A = 1 << 2 | 27 << 8 | 0<<14;
 ...
 DMANow(3, hooksTiles, &((charblock *)0x6000000)[0], 10624/2);
 DMANow(3, hooksMap, &((screenblock *)0x6000000)[31], 2048/2);

 DMANow(3, seaTiles, &((charblock *)0x6000000)[1], 4640/2);
 loadMap(seaMap, 2048, 0, 27);
 ...
 *(volatile unsigned short*)0x4000008 = 0 << 2 | 31 << 8 | 0<<14;
 ...
 DMANow(3, instructionsTiles, &((charblock *)0x6000000)[0], 10720/2);
 ...
 *(volatile unsigned short*)0x4000008 = 0 << 2 | 31 << 8 | 0<<14;
 ...
 DMANow(3, instructions2Tiles, &((charblock *)0x6000000)[0], 11520/2);
 ...
 *(volatile unsigned short*)0x4000008 = 0 << 2 | 31 << 8 | 0<<14;
 ...
 DMANow(3, instructions3Tiles, &((charblock *)0x6000000)[0], 8000/2);
 ...
 *(volatile unsigned short*)0x4000008 = 0 << 2 | 31 << 8 | 0<<14;
 ...
 DMANow(3, pauseTiles, &((charblock *)0x6000000)[0], 6496/2);
 ...
 *(volatile unsigned short*)0x4000008 = 0 << 2 | 31 << 8 | 0<<14 | 1 << 7;
 ...
 loadPalette(losePal);
 DMANow(3, loseTiles, &((charblock *)0x6000000)[0], 35648/2);
